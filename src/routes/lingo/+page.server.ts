...
import { ROUNDS_PER_GAME, LINGO_WORD_LENGTH } from '$lib/config/constants';
...
export const load: PageServerLoad = async ({ url, locals }) => {
	...
	for (let r = 1; r <= ROUNDS_PER_GAME; r++) {
		words.push(getWord(LINGO_WORD_LENGTH, gameIndex * ROUNDS_PER_GAME + r));
	}
		...
		const result = await db
			...
			.where(
				and(
					...
					eq(scores.wordLength, LINGO_WORD_LENGTH)
				)
			)
			.limit(1);
	...
	saveScore: async ({ request, locals }) => {
		...
		if (existing.length === 0) {
			await db.insert(scores).values({
				...
				wins: won ? 1 : 0,
				losses: won ? 0 : 1,
				totalScore: wordsGuessed * 10 + (won ? 25 : 0),
			...
			await db
				...
				.set({
					...
					totalScore: s.totalScore + wordsGuessed * 10 + (won ? 25 : 0),
